# Kết quả Submission - 16:58 09/07/2026 (STT 4 - Baseline GS 30k Iterations - Run 3 / Test Sai Số)
...
- **Mục đích**: Thử nghiệm lặp lại cấu hình mặc định (lần thứ 3) nhằm đo lường sai số và độ biến động kết quả (grading variance) của hệ thống chấm bài.
...
1. **Phân tích sai số khi chấm bài (so sánh với Run 1 và Run 2)**:
   - Điểm số của Run 3 (`52.48840`) giảm rất mạnh so với Run 1 (`55.65110` - giảm 3.16 điểm) và Run 2 (`54.97630` - giảm 2.49 điểm).
   - Sự sụt giảm xảy ra đồng đều ở tất cả chỉ số: PSNR giảm từ `19.49dB` xuống `18.39dB` (giảm 1.1dB), SSIM giảm xuống `52.08%`, LPIPS tăng lên `35.42%`.
