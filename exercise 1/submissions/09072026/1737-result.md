# Kết quả Submission - 17:37 09/07/2026 (STT 6 - Baseline GS 15k Iterations - Run 5 / Test Sai Số)
...
- **Mục đích**: Thử nghiệm lặp lại cấu hình mặc định (lần thứ 5) nhằm thu thập thêm dữ liệu phân tích về sai số và độ biến động kết quả của hệ thống chấm bài.
...
1. **Phân tích sai số khi chấm bài (so sánh với các Run 1-4)**:
   - Điểm số của Run 5 (`53.87740`) nằm rất gần với Run 4 (`53.77280`).
   - LPIPS (`34.17%`) và PSNR (`19.17dB`) cũng tương đương với Run 4, tiếp tục củng cố mức điểm trung bình ổn định của baseline mặc định quanh khoảng 53-54 điểm.
2. **Kết luận**:
