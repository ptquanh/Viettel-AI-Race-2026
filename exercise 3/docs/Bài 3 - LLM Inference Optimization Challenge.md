...
- **Runtime & Compiler Optimization:** Chỉ sử dụng framework vLLM (hệ thống grader bắt buộc khởi chạy bằng vLLM entrypoint và không tương thích/hỗ trợ các serving engine khác như SGLang, LMDeploy, Aphrodite, v.v., các đội không cần thử nghiệm các framework khác); tùy chỉnh tensor parallelism và pipeline parallelism; overlap communication/computation.
