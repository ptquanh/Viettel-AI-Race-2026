...
Thí sinh **bắt buộc chỉ được sử dụng framework vLLM** (hệ thống chấm bài tự động ép buộc entrypoint của vLLM và không hỗ trợ/tương thích với các framework khác như SGLang, LMDeploy, Aphrodite, v.v.). Do đó, không cần thử nghiệm các framework serving khác để tránh lặp lại lỗi startup/timeout. Các hướng tiếp cận cốt lõi bao gồm:
