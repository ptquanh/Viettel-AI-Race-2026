...
print("[Antigravity Phase 5 v18.0] Triton JIT Kernel + Modern vLLM Engine Initialized", file=sys.stderr)

try:
    import vllm.model_executor.models.lfm as lfm_module
    print("[Antigravity Phase 5 v18.0] Bound hooks to vllm.model_executor.models.lfm!", file=sys.stderr)
except Exception as e:
    print(f"[Antigravity Phase 5 v18.0] LFM hook binding status: {e}", file=sys.stderr)
...
try:
    ...
    def _patched_forward_cuda(self, hidden_states, output):
        try:
            ...
            import sys
            print(f"[Antigravity v18.0] Triton Kernel Warning: {patch_e}, fallback to orig!", file=sys.stderr)
            return _orig_forward_cuda(self, hidden_states, output)

    ShortConv.forward_cuda = _patched_forward_cuda
    print("[Antigravity Phase 5 v18.0] Successfully patched ShortConv.forward_cuda with TRITON KERNEL!", file=sys.stderr)
except Exception as e:
    print(f"[Antigravity Phase 5 v18.0] Error patching ShortConv: {e}", file=sys.stderr)
...
try:
    ...
    def _patched_from_engine_args(cls, engine_args, **kwargs):
        ...
        print("[Antigravity Phase 5 v18.0] Modern Engine initialized! Executing Native Zero-Penalty JIT Warmup...", file=sys.stderr)
            ...
            try:
                ...
                for i in range(warmup_rounds):
                    ...
                    async for _ in results_generator:
                        pass
                print(f"[Antigravity Phase 5 v18.0] Native Zero-Penalty Warmup COMPLETE ({warmup_rounds} rounds)!", file=sys.stderr)
            except Exception as w_err:
                print(f"[Antigravity Phase 5 v18.0] Native Warmup Warning (non-fatal): {w_err}", file=sys.stderr)
        ...
        try:
            ...
            print(f"[Antigravity Phase 5 v18.0] Asyncio Loop execution note: {l_err}", file=sys.stderr)

        return engine

    AsyncLLMEngine.from_engine_args = _patched_from_engine_args
    print("[Antigravity Phase 5 v18.0] Successfully installed AsyncLLMEngine native warmup patch!", file=sys.stderr)

except Exception as hook_err:
    print(f"[Antigravity Phase 5 v18.0] Native Warmup Hook Error: {hook_err}", file=sys.stderr)
