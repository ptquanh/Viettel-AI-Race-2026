...
def _fused_short_conv_update_kernel(
    ...
    b = tl.load(b_ptr, mask=mask_dim, other=0.0)
    c = tl.load(c_ptr, mask=mask_dim, other=0.0)
    x = tl.load(x_ptr, mask=mask_dim, other=0.0)
    ...
    for i in range(L_cache - 1):
        val = tl.load(state_base + dim_offset * stride_state_dim + (i + 1) * stride_state_l, mask=mask_dim, other=0.0)
        tl.store(state_base + dim_offset * stride_state_dim + i * stride_state_l, val, mask=mask_dim)
        
        w = tl.load(weights_ptr + dim_offset * stride_weights_dim + i * stride_weights_l, mask=mask_dim, other=0.0)
    ...
    w_last = tl.load(weights_ptr + dim_offset * stride_weights_dim + (L_cache - 1) * stride_weights_l, mask=mask_dim, other=0.0)
    dot_acc += bx_f32 * w_last.to(tl.float32)
    
    if HAS_BIAS:
        bias_val = tl.load(bias_ptr + dim_offset, mask=mask_dim, other=0.0)
        dot_acc += bias_val.to(tl.float32)
...
def fused_lfm_short_conv_update(bcx, conv_state, conv_weights, conv_bias, state_indices, out_buffer=None):
    num_tokens = bcx.size(0)
    dim = conv_weights.size(0)
    L_cache = conv_weights.size(1)
    
    if out_buffer is not None and out_buffer.size(0) >= num_tokens and out_buffer.size(1) == dim:
        y = out_buffer[:num_tokens]
    else:
        y = torch.empty((num_tokens, dim), device=bcx.device, dtype=bcx.dtype)
    
    has_bias = conv_bias is not None and conv_bias.numel() > 0
    bias_ptr = conv_bias if has_bias else bcx  # dummy pointer if no bias
    ...
    _fused_short_conv_update_kernel[grid](
        bcx, conv_state, conv_weights, bias_ptr, y, state_indices,
